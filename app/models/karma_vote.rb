class KarmaVote < ActiveRecord::Base
  belongs_to :user
  ...
  class << self
    ...
    def change(user, another_user, change_value)
      if another_user == self
        ...
        my_vote = KarmaVote.find_or_initialize_by_user_id_and_affected_id(user.id, another_user.id)
