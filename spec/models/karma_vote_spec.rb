...
describe KarmaVote do
  fixtures :karma_votes, :users

  before :each do
    @vote = KarmaVote.find 1 
  ...
  def stored_for(user, affected_user)
    v = KarmaVote.find_by_user_id_and_affected_id(user.id, affected_user.id)
    v && v.value
  end
  
  describe "#change" do
    it "changes stored value by the required value" do
      lambda { KarmaVote.change(@user1, @user2, 100) }.should change { stored_for(@user1, @user2) }.from(5).to(105)
    end
    
    it "initializes value by the required value if value not stored" do
      lambda { KarmaVote.change(@user3, @user4, 100) }.should change { stored_for(@user3, @user4) }.from(nil).to(100)
    end
  end
  
  describe "#take_back_all_from" do
    it "should remove all votes stored for user" do
      lambda { KarmaVote.take_back_all_from(@user1) }.should change { stored_for(@user1, @user2) }.from(5).to(nil)
    end
  end
end
